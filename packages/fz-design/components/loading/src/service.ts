...
import { addClass, getStyle, isClient, isString, removeClass } from '../../../utils'
import { createLoadingComponent } from './loading'

import type { UseNamespaceReturn, UseZIndexReturn } from '../../../hooks'
...
export const Loading = function (options: LoadingOptions = {}): LoadingInstance {
  ...
  resolved.parent.vLoadingAddClassList = (): void =>
    addClassList(resolved, resolved.parent, instance)
...
const addStyle = async (
  ...
  if (options.fullscreen) {
    ...
    for (const property of ['top', 'left']) {
      ...
      const rect = (options.target as HTMLElement).getBoundingClientRect() as any
      ...
      maskStyle[property as any] = `${val}px`
    }
    for (const property of ['height', 'width']) {
      const rect = (options.target as HTMLElement).getBoundingClientRect() as any
      const val = rect[property]
      maskStyle[property as any] = `${val}px`
    }
  } else {
...
const addClassList = (
  ...
  // Compatible with the instance data format of vue@3.2.12 and earlier versions #12351
  const ns =
    ((instance.vm as any).ns as UseNamespaceReturn) || (instance.vm as any)._.exposed.ns
