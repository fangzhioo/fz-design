<script lang="ts" setup>
  ...
  import { addUnit, getScrollContainer, throwError } from '../../../utils'
  ...
  onMounted(() => {
    if (props.target) {
      ...
      if (!target.value) throwError(ns.b(), `Target is not existed: ${props.target}`)
    } else {
      target.value = document.documentElement
    }
    scrollContainer.value = getScrollContainer(root.value!, true)
    // console.log(scrollContainer.value, 's');
