...
import { camelize, isClient, isNumber, isString, warning } from '..'

const trimArr = (s: string): string[] => (s || '').split(' ').filter(item => Boolean(item.trim()))
...
export function addUnit (value: string | number, unit = 'px'): string {
  ...
  warning('addUnit', 'binding value must be a string or number')
  return ''
}
...
export function addClass (el: HTMLElement | Element, cls: string): void {
  ...
  const classes = (cls || '')
    .split(' ')
    .filter(item => !curClass.includes(item) && Boolean(item.trim()))
...
export const getClientXY = (event: MouseEvent | TouchEvent | Event): { clientX: number; clientY: number } => {
  let clientX: number
  let clientY: number
