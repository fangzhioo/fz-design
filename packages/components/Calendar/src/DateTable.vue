  ...
  >
    ...
    <tbody>
      <tr
        ...
        :key="index"
        :class="{
          'fz-calendar-table__row': true,
          'fz-calendar-table__row--hide-border': index === 0 && hideHeader,
        }"
      >
        <td v-for="(cell, key) in row" :key="key" :class="getCellClass(cell)" @click="handlePickDay(cell)">
          <div class="fz-calendar-day">
...
import { isObject, rangeArr } from '@fzui/utils';
...
export default defineComponent({
  ...
  setup(props, { emit }) {
    ...
    const isInRange = computed(() => Boolean(props.range) && Boolean(props.range?.length));
    ...
    const rows = computed(() => {
      ...
      if (isInRange.value) {
        ...
        const prevMonthDays: Cell[] = getPrevMonthLastDays(props.date, firstDay - firstDayOfWeek).map((day) => ({
          text: day,
          type: 'prev',
        }));
    ...
    const weekDays = computed(() => {
      ...
      if (start === 0) {
        return WEEK_DAYS.map((_) => t(`fz.datepicker.weeks.${_}`));
      }
      return WEEK_DAYS.slice(start)
        .concat(WEEK_DAYS.slice(0, start))
        .map((_) => t(`fz.datepicker.weeks.${_}`));
    });
