<template>
  <table role="grid" :aria-label="t('fz.datepicker.yearTablePrompt')" class="fz-year-table" @click="handleYearTableClick">
    <tbody ref="tbodyRef">
      <tr v-for="(n, i) in 3" :key="i">
        <template v-for="(nn, j) in 4" :key="i + '_' + j">
          <td
            v-if="i * 4 + j < 10"
            :ref="(el) => isSelectedCell(startYear + i * 4 + j) && (currentCellRef = el)"
            class="available"
