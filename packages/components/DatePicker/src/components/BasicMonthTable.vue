<template>
  <table role="grid" :aria-label="t('fz.datepicker.monthTablePrompt')" class="fz-month-table" @click="handleMonthTableClick" @mousemove="handleMouseMove">
    <tbody ref="tbodyRef">
      <tr v-for="(row, key) in rows" :key="key">
        <td
          ...
          :aria-label="t(`fz.datepicker.month${Number(cell.text) + 1}`)"
        ...
        >
          <div>
            <span class="cell">
              {{ t('fz.datepicker.months.' + months[Number(cell.text)]) }}
            </span>
          </div>
        </td>
      </tr>
...
import { DateCell } from '../DatePicker';
...
export default defineComponent({
  ...
  setup(props, ctx) {
    ...
    const rows = computed<DateCell[][]>(() => {
      ...
      for (let i = 0; i < 3; i++) {
        const row: DateCell[] = rows[i];
        for (let j = 0; j < 4; j++) {
          let cell: DateCell = row[j];
          if (!cell) {
            cell = {
              row: i,
              column: j,
          ...
          cell.inRange =
            (props.minDate && calTime.isSameOrAfter(props.minDate, 'month') && calEndDate && calTime.isSameOrBefore(calEndDate, 'month')) ||
            (props.minDate && calTime.isSameOrBefore(props.minDate, 'month') && calEndDate && calTime.isSameOrAfter(calEndDate, 'month'));
          ...
          if (props.minDate?.isSameOrAfter(calEndDate)) {
            ...
            if (props.minDate) {
              cell.start = calTime.isSame(props.minDate, 'month');
            }
