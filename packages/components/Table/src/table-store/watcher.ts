...
import { hasOwn } from '@fzui/utils';
import { getKeysMap, getRowIdentity, getColumnById, getColumnByKey, orderBy, toggleRowStatus } from '../table-utils/util';
...
import type { Table, TableRefs } from '../Table';
import type { StoreFilter } from './index';

const sortData = (data: any, states: any) => {
...
const doFlattenColumns = (columns: any[]) => {
  const result: any[] = [];
...
function useWatcher<T>() {
  ...
  const sortingColumn = ref<any>(null);
  const sortProp = ref<any>(null);
  ...
  const updateColumns = () => {
    ...
    originColumns.value = []
      .concat(fixedColumns.value as any)
      .concat(notFixedColumns as any)
      .concat(rightFixedColumns.value as any);
    ...
    columns.value = []
      .concat(fixedLeafColumns as any)
      .concat(leafColumns as any)
      .concat(rightFixedLeafColumns as any);
  ...
  const toggleRowSelection = (row: T, selected: boolean | undefined = undefined, emitChange = true) => {
  ...
  const updateAllSelected = () => {
    ...
    const isSelected = (row: any) => {
      if (selectedMap) {
        return Boolean(selectedMap[getRowIdentity(row, rowKey.value)]);
      }
      return selection.value.indexOf(row) !== -1;
    };
  ...
  const getChildrenCount = (rowKey: string) => {
    ...
    let count = 0;
    const children: any[] = treeData.value[rowKey]?.children;
    if (children) {
  ...
  // 过滤与排序
  const updateFilters = (cols: any[], values: any[]) => {
    let columns = cols;
  ...
  const clearFilter = (colKeys: any[]) => {
    let columnKeys = colKeys;
    const { tableHeader, fixedTableHeader, rightFixedTableHeader } = instance.refs as TableRefs;
    let panels = {};
    if (tableHeader) {
