...
import { hasOwn, isClient } from '@fzui/utils';
...
class TableLayout<T> {
  ...
  constructor(options: Record<string, any>) {
    ...
    for (const name in options) {
      if (hasOwn(options, name)) {
        if (isRef(this[name])) {
          (this[name] as any).value = options[name];
        } else {
          (this[name] as any) = options[name];
        }
      }
    }
    if (!this.table) {
  ...
  setHeight(val: string | number, prop = 'height'): any {
    let value: any = val;
    if (!isClient) {
      return;
    }
  ...
  updateColumnsWidth() {
    ...
    const flattenColumns: any[] = this.getFlattenColumns();
    ...
    const fixedColumns: any[] = this.store.states.fixedColumns.value;
    ...
    const rightFixedColumns: any[] = this.store.states.rightFixedColumns.value;
