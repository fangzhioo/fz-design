...
import { hasClass, addClass, removeClass } from '@fzui/utils';
import { FZ_TABLE_INJECT_KEY } from '@fzui/constants';
import type { TableHeaderProps } from '.';
import type { TableColumnCtx } from '../table-column/defaults';

function useEvent<T>(props: TableHeaderProps<T>, emit: any) {
  ...
  const toggleOrder = ({ order, sortOrders }: any) => {
    if (order === '') {
      return sortOrders[0];
    }
  ...
  const handleSortClick = (event: Event, column: TableColumnCtx<T>, givenOrder: string | boolean) => {
    event.stopPropagation();
