...
const hsv2hsl = (hueVal: number, sat: number, val: number) => {
  let hue = hueVal;
...
const bound01 = (val: number | string, max: number | string) => {
  let value = val;
...
const parseHexChannel = (hex: string) => {
  if (hex.length === 2) {
    return (HEX_INT_MAP[hex[0].toUpperCase()] || Number(hex[0])) * 16 + (HEX_INT_MAP[hex[1].toUpperCase()] || Number(hex[1]));
  }

  return HEX_INT_MAP[hex[1].toUpperCase()] || Number(hex[1]);
};

const hsl2hsv = (hue: number, satVal: number, lightVal: number) => {
  let sat = satVal;
  let light = lightVal;
...
const rgb2hsv = (red: number, green: number, blue: number) => {
  let r = red;
  let g = green;
  let b = blue;
  r = bound01(r, 255);
...
// `hsvToRgb` 色调（H），饱和度（S），明度（V）
// Converts an HSV color value to RGB.
// *Assumes:* h is contained in [0, 1] or [0, 360] and s and v are contained in [0, 1] or [0, 100]
// *Returns:* { r, g, b } in the set [0, 255]
const hsv2rgb = (hVal: number, sVal: number, vVal: number) => {
  let h = hVal;
  let s = sVal;
  let v = vVal;
  h = bound01(h, 360) * 6;
...
export default class Color {
  ...
  constructor(optionsVal?: Options) {
    let options = optionsVal;
  ...
  fromString(value: any) {
    ...
    if (value.indexOf('hsl') !== -1) {
      const parts = value
        ...
        .map((val: string, index: number) => (index > 2 ? parseFloat(val) : parseInt(val, 10)));
