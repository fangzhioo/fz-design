<template>
  <transition :name="transitionName">
    <div v-if="actualVisible || visible" class="fz-time-panel">
      <div class="fz-time-panel__content" :class="{ 'has-seconds': showSeconds }">
        <time-spinner
          ref="spinner"
      ...
      <div class="fz-time-panel__footer">
        ...
        <button type="button" class="fz-time-panel__btn confirm" @click="handleConfirm()">
...
export default defineComponent({
  ...
  props: {
    ...
    parsedValue: {
      type: [Object, String] as PropType<Dayjs>,
    },
    format: {
      type: String,
      default: '',
  ...
  setup(props, ctx) {
    ...
    const transitionName = computed(() => {
      return props.actualVisible === undefined ? 'fz-zoom-in-top' : '';
    });
    ...
    const setSelectionRange = (start: any, end: any) => {
      ...
      ['hour', 'minute', 'second'].forEach((k) => {
        if (availableMap[k]) {
          ...
          if (availableArr && availableArr.length && !availableArr.includes(result[k]())) {
            result = result[k](availableArr[0]);
          }
        }
      });
      return result;
