...
import type { Filter, Sort, Table } from '../Table';
...
function replaceColumn<T>(array: TableColumnCtx<T>[], column: TableColumnCtx<T>) {
...
function useStore<T>(): any {
  ...
  const mutations = {
    ...
    removeColumn(states: StoreStates, column: TableColumnCtx<T>, parent: TableColumnCtx<T> & any) {
      const array = unref(states._columns) || [];
      if (parent) {
        parent.children.splice(
          parent.children.findIndex((item: any) => item.id === column.id),
          1,
        );
    ...
    rowSelectedChanged(_states: any, row: T) {
    ...
    setCurrentRow(_states: any, row: T) {
