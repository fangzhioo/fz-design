...
import { debounce, getStyle, hasClass } from '@fzui/utils';
import { createTablePopper, getCell, getColumnByCell } from '../table-utils/util';
...
function useEvents<T>(props: Partial<TableBodyProps<T>>) {
  ...
  const handleEvent = (event: Event, row: T, name: string) => {
    ...
    table?.emit(`row-${name}`, row, undefined, event);
  };
  ...
  const handleCellMouseEnter = (event: MouseEvent, row: T & { tooltipEffect: string }) => {
    const table = parent;
    const cell = getCell(event)!;
    const namespace = table?.vnode.el?.dataset.prefix;
    if (cell) {
