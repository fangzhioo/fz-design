...
function useStyle<T>(props: TableProps<T>, layout: TableLayout<T>, store: Store<T>, table: Table<T>) {
  ...
  const handleHeaderFooterMousewheel = (event: any, data: any) => {
  ...
  const syncPostion = function () {
    ...
    const scrollContainer = table.refs.scrollWrapper.wrapRef;
    if (!scrollContainer) {
      return;
    }
   ...
   * fix layout
   */
  const handleFixedMousewheel = (event: any, data: any) => {
