...
import { hasOwn, off, on } from '@fzui/utils';
import { useZIndex } from '@fzui/hooks';
// import type { PopperInstance, IPopperOptions } from '@fzui/components/popper';
import type { Nullable } from '@fzui/utils';
import type { TableColumnCtx } from '../table-column/defaults';

type PopperInstance = any;
type IPopperOptions = any;

export const getCell = function (event: Event): HTMLElement | null {
...
export const orderBy = function <T>(
  ...
  sortMethod: any,
  _sortBy: string | (string | ((a: T, b: T, array?: T[]) => number))[],
) {
...
export function parseWidth(w: number | string): number | string {
  let width = w;
  if (width === '') {
    return width;
  }
...
export function parseMinWidth(minW: number | string): number | string {
  let minWidth = minW;
  if (minWidth === '') {
    return minWidth;
  }
...
export function toggleRowStatus<T>(statusArr: T[], row: T, newVal?: boolean): boolean {
  let changed = false;
...
export function walkTreeNode(root: any[], cb: any, childrenKey = 'children', lazyKey = 'hasChildren') {
  const isNil = (array: any) => !(Array.isArray(array) && array.length);

  function _walker(parent: any, children: any[], level: any) {
...
export let removePopper: { (): void; (): void; (args: any): void };

export function createTablePopper(trigger: HTMLElement, popperContent: string, popperOptions: Partial<IPopperOptions>, tooltipEffect: string) {
  const { nextZIndex } = useZIndex();
  function renderContent(): HTMLDivElement {
    const isLight = tooltipEffect === 'light';
    const content = document.createElement('div');
    content.className = `fz-popper ${isLight ? 'is-light' : 'is-dark'}`;
  ...
  function renderArrow(): HTMLDivElement {
    ...
    arrow.className = 'fz-popper__arrow';
    return arrow;
  }
  function showPopper() {
...
export const isFixedColumn = <T>(index: number, fixed: string | boolean, store: any, realColumns?: TableColumnCtx<T>[]) => {
  let start = 0;
  let after = index;
  if (realColumns) {
...
export const getFixedColumnsClass = <T>(namespace: string, index: number, fixed: string | boolean, store: any, realColumns?: TableColumnCtx<T>[]) => {
...
export const getFixedColumnOffset = <T>(index: number, fixed: string | boolean, store: any, realColumns?: TableColumnCtx<T>[]) => {
