...
import { EVENT_CODE, FZ_SLIDER_INJECT_KEY, UPDATE_MODEL_EVENT } from '@fzui/constants';

import type { CSSProperties, ComponentInternalInstance, ComputedRef } from 'vue';
import type { ISliderButtonInitData, ISliderButtonProps, ISliderProvider } from '../Slider';
...
export const useSliderButton = (props: ISliderButtonProps, initData: ISliderButtonInitData, emit: ComponentInternalInstance['emit']) => {
  const { disabled, min, max, step, showTooltip, precision, sliderSize, formatTooltip, emitChange, resetSize, updateDragging } =
    inject<ISliderProvider>(FZ_SLIDER_INJECT_KEY)!;
