 ...
 */
export const isValidElementNode = (node: unknown): node is VNode => isVNode(node) && !isFragment(node) && !isComment(node);
...
export function renderIf(condition: boolean, node: VNodeTypes, props: any, children?: Children, patchFlag?: number, patchProps?: string[]) {
  return condition ? renderBlock(node, props, children, patchFlag, patchProps) : createCommentVNode('v-if', true);
}

export function renderBlock(node: VNodeTypes, props: any, children?: Children, patchFlag?: number, patchProps?: string[]) {
  openBlock();
  return createBlock(node, props, children, patchFlag, patchProps);
}
