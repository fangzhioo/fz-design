...
(function(global2, factory) {
  ...
  const FZ_CONFIG_PROVIDER_INJECT_KEY = Symbol("config-provider");
  const FZ_LOCALE_INJECT_KEY = Symbol("locale");
  const FZ_BUTTON_GROUP_INJECT_KEY = Symbol("button-group");
  const FZ_FORM_INJECT_KEY = Symbol("form");
  const FZ_FORMITEM_INJECT_KEY = Symbol("form-item");
  const useForm = () => {
    const form = vue.inject(FZ_FORM_INJECT_KEY, void 0);
    const formItem = vue.inject(FZ_FORMITEM_INJECT_KEY, void 0);
    return {
      form,
      formItem
    };
  ...
  function useGlobalConfig(key) {
    const config = vue.inject(FZ_CONFIG_PROVIDER_INJECT_KEY, {});
    if (key) {
  ...
  const translate = (path, option, locale) => lodash.exports.get(locale, path, path).replace(/\{(\w+)\}/g, (_, key) => {
    var _a;
    return `${(_a = option == null ? void 0 : option[key]) != null ? _a : `{${key}}`}`;
  });
  const buildTranslator = (locale) => (path, option) => translate(path, option, vue.unref(locale));
  const provideLocale = () => {
    const propLocale = useProp("locale");
    const locale = vue.computed(() => propLocale.value || English);
  ...
  const useSize = (fallback, ignore = {}) => {
    ...
    const form = ignore.form ? { size: void 0 } : vue.inject(FZ_FORM_INJECT_KEY, void 0);
    const formItem = ignore.formItem ? { size: void 0 } : vue.inject(FZ_FORMITEM_INJECT_KEY, void 0);
    return vue.computed(() => size.value || vue.unref(fallback) || (formItem == null ? void 0 : formItem.size) || (form == null ? void 0 : form.size) || globalConfigSize.value || "");
  };
  ...
  const useTheme = (fallback) => {
    var _a;
    const propTheme = useProp("theme");
    if (propTheme) {
      return propTheme;
    }
    const theme = vue.ref((_a = vue.unref(fallback)) != null ? _a : "light");
    if (isClient) {
  ...
  var script$8 = vue.defineComponent({
    ...
    setup(props, { slots, emit }) {
      ...
      const buttonGroupContext = vue.inject(FZ_BUTTON_GROUP_INJECT_KEY, void 0);
      const autoInsertSpace = vue.computed(() => props.autoInsertSpace);
      ...
      const buttonDisabled = vue.computed(() => props.disabled);
...
`;
  ...
  var script$6 = vue.defineComponent({
    ...
    setup(props, { slots, emit, attrs: rawAttrs }) {
      ...
      const computedTextareaStyle = vue.computed(() => [props.inputStyle, textareaCalcStyle.value, { resize: props.resize }]);
      const nativeInputValue = vue.computed(() => props.modelValue === null || props.modelValue === void 0 ? "" : String(props.modelValue));
      const showClear = vue.computed(() => props.clearable && !inputDisabled.value && !props.readonly && Boolean(nativeInputValue.value) && (focused.value || hovering.value));
      const showPwdVisible = vue.computed(() => props.showPassword && !inputDisabled.value && !props.readonly && (Boolean(nativeInputValue.value) || focused.value));
      const isWordLimitVisible = vue.computed(() => props.showWordLimit && Boolean(attrs.value.maxlength) && (props.type === "text" || props.type === "textarea") && !inputDisabled.value && !props.readonly && !props.showPassword);
      const textLength = vue.computed(() => Array.from(nativeInputValue.value).length);
      const inputExceed = vue.computed(() => Boolean(isWordLimitVisible.value) && textLength.value > Number(attrs.value.maxlength));
      const suffixVisible = vue.computed(() => Boolean(slots.suffix) || Boolean(props.suffixIcon) || showClear.value || props.showPassword || isWordLimitVisible.value || Boolean(validateState.value) && needStatusIcon.value);
      ...
      const handleCompositionUpdate = (event) => {
        ...
        const text = target == null ? void 0 : target.value;
  ...
  const FzUIPlugin = {
    install(app) {
      ...
      (_b = (_a = ButtonPlugin).install) == null ? void 0 : _b.call(_a, app);
      (_d = (_c = InputPlugin).install) == null ? void 0 : _d.call(_c, app);
      (_f = (_e = ConfigProviderPlugin).install) == null ? void 0 : _f.call(_e, app);
      (_h = (_g = ContainerPlugin).install) == null ? void 0 : _h.call(_g, app);
    }
  };
  var style = "";
  return FzUIPlugin;
