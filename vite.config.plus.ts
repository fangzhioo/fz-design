...
import { name, version } from './packages/fz-design-plus/package.json'
  ...
  return {
    ...
    plugins: [
      ...
      dts({
        ...
        include: ['./packages/fz-design-plus'] /** 手动设置包含路径的 glob */,
        /** 构建后回调钩子 */
        afterBuild: (): void => {
          move()
        }
    ...
    build: {
      ...
      lib: {
        name: 'FzDesignPlus',
        entry: resolve(__dirname, 'packages/fz-design-plus/index.ts') /** 打包入口 */
      },
      /**
       * rollup 配置项
       *
...
const move = (): void => {
  const files = [
    ...
    { input: './packages/fz-design-plus/package.json', outDir: 'dist/package.json' },
